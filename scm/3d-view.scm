...
(define-method (draw (portal <portal>))
  ...
  (or (and-let* ((passage #[portal 'passage])
		 ((is-a? passage <passage>))
		 (its-objects #[passage 'objects]))
	;;(<< `(valid portal ,portal))
	(for-each draw its-objects)
	(let ((other-side (other-side portal)))
	  (rotate-view! (~ #[other-side 'orientation]))
	  (translate-view! (- #[other-side 'position]))
	  (or (and-let* ((opposite-sector #[other-side 'context])
			 ((is-a? opposite-sector <subspace>))
			 (its-objects #[opposite-sector 'objects]))
		;;(<< `(valid portal ,other-side))
		(for-each draw (delete other-side its-objects)))
	      (throw 'invalid-portal other-side))))
